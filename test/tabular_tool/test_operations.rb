...
class TestOperations < Minitest::Test
  ...
  def test_sample_on_lazy_frame
    # Test sampling on a lazy frame (like when reading parquet files)
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))
    assert lf.is_a?(Polars::LazyFrame), "Should be a lazy frame"

    # This will fail with current implementation because it tries to collect all data
    # After fix, it should work efficiently by taking a subset first
    result = TabularTool::Operations.sample(lf, n: 5)

    assert result.is_a?(Polars::DataFrame), "Should return a DataFrame"
    assert_equal 5, result.height, "Should have 5 rows"
  end

  def test_sample_lazy_frame_with_fraction
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))

    result = TabularTool::Operations.sample(lf, fraction: 0.5)

    assert result.is_a?(Polars::DataFrame), "Should return a DataFrame"
    assert_operator result.height, :>, 0, "Should have some rows"
    assert_operator result.height, :<=, 10, "Should not exceed total rows"
  end

  def test_sample_lazy_frame_without_replacement
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))

    # Sample all rows -- with-replacement could produce duplicates,
    # without-replacement must return exactly the original rows
    result = TabularTool::Operations.sample(lf, n: 10)

    assert_equal 10, result.height
    # Each row index should appear exactly once (no duplicates from replacement)
    names = result["name"].to_a.sort
    assert_equal names.uniq, names, "Sample should not contain duplicate rows (without-replacement)"
  end

  def test_tail_on_lazy_frame
    # Test tailing on a lazy frame (like when reading parquet files)
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))
    assert lf.is_a?(Polars::LazyFrame), "Should be a lazy frame"

    # LazyFrame has .tail() method, so this should work efficiently
    result = TabularTool::Operations.tail(lf, n: 3)

    assert result.is_a?(Polars::DataFrame), "Should return a DataFrame"
    assert_equal 3, result.height, "Should have 3 rows"
  end

  def test_tail_lazy_frame_default
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))

    result = TabularTool::Operations.tail(lf)

    assert result.is_a?(Polars::DataFrame), "Should return a DataFrame"
    assert_equal 10, result.height, "Should have 10 rows (default)"
  end

  def test_head_on_lazy_frame
    # Test heading on a lazy frame (like when reading parquet files)
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))
    assert lf.is_a?(Polars::LazyFrame), "Should be a lazy frame"

    # LazyFrame has .head() method, so this should work efficiently
    result = TabularTool::Operations.head(lf, n: 3)

    assert result.is_a?(Polars::DataFrame), "Should return a DataFrame"
    assert_equal 3, result.height, "Should have 3 rows"
  end

  def test_head_lazy_frame_default
    lf = Polars.scan_parquet(File.join(@fixtures_path, "basic.parquet"))

    result = TabularTool::Operations.head(lf)
