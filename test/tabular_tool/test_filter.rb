...
class TestFilter < Minitest::Test
  ...
  def test_dsl_translates_equality
    assert_equal "status = 'active'", dsl_translate("status == 'active'")
  end

  def test_dsl_translates_inequality
    assert_equal "status <> 'active'", dsl_translate("status != 'active'")
  end

  def test_dsl_translates_and_or
    assert_equal "age > 30  AND  status = 'active'",
      dsl_translate("age > 30 && status == 'active'")
    assert_equal "age < 25  OR  age > 35",
      dsl_translate("age < 25 || age > 35")
  end

  def test_dsl_translates_not
    assert_equal "NOT (status = 'inactive')", dsl_translate("!(status == 'inactive')")
  end

  def test_dsl_translates_contains
    assert_equal "city LIKE '%San%'", dsl_translate("city.contains('San')")
  end

  def test_dsl_translates_starts_with
    assert_equal "name LIKE 'A%'", dsl_translate("name.starts_with('A')")
  end

  def test_dsl_translates_ends_with
    assert_equal "name LIKE '%e'", dsl_translate("name.ends_with('e')")
  end

  def test_dsl_translates_is_null
    assert_equal "email IS NULL", dsl_translate("email.is_null()")
    assert_equal "email IS NOT NULL", dsl_translate("email.is_not_null()")
  end

  def test_dsl_escapes_like_wildcards
    assert_equal "col LIKE '%10\\%%'", dsl_translate("col.contains('10%')")
    assert_equal "col LIKE '%a\\_b%'", dsl_translate("col.contains('a_b')")
  end

  def test_dsl_translates_double_quotes_to_single
    assert_equal "status = 'active'", dsl_translate('status == "active"')
  end

  def test_dsl_inequality_does_not_corrupt_adjacent_equals
    # Ensure != translation doesn't interfere with standalone ==
    result = dsl_translate("a != 'x'  AND  b == 'y'")
    assert_includes result, "a <> 'x'"
    assert_includes result, "b = 'y'"
  end

  def test_dsl_preserves_operators_inside_string_literals
    assert_equal "col = 'a==b'", dsl_translate("col == 'a==b'")
    assert_equal "col = 'a!=b'", dsl_translate("col == 'a!=b'")
    assert_equal "col = 'a&&b'", dsl_translate("col == 'a&&b'")
  end
  ...
  def dsl_translate(expression)
    TabularTool::Operations::DslToSql.new(expression).translate
  end
end
