...
module TabularTool
  module Operations
    ...
    #
    # Note: double-quoted strings are treated as string literals (converted to
    # single-quoted SQL). To reference column names with special characters,
    # use unquoted identifiers (Polars handles most cases).
    class DslToSql
      ...
      def translate
        ...
        sql = translate_operators_outside_strings(sql)
      ...
      def translate_operators_outside_strings(sql)
        # Split into string-literal and non-literal segments to avoid
        # corrupting operators that appear inside quoted values
        parts = sql.split(/('(?:[^'\\]|\\.)*')/)

        parts.map.with_index do |part, i|
          if i.odd?
            part  # string literal -- leave untouched
          else
            translate_operators(part)
          end
        end.join
      end
