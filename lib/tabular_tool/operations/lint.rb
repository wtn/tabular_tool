  ...
  module Operations
    module Lint
      def self.call(df, unique_columns: nil)
        ...
        # Optional: check for duplicate values in specific columns
        if unique_columns
      ...
      def self.check_duplicate_rows(df, errors)
        ...
        # Filter to only groups with more than one occurrence
        duplicates = grouped.filter(Polars.col("indices").list.len.gt(1))

        if duplicates.height > 0
          # Collect all duplicate row indices
          all_dup_indices = []
      ...
      def self.check_whitespace(df, warnings)
        df.columns.each do |col|
          ...
          begin
            ...
            # Filter rows where the column has leading or trailing whitespace
            ...
            # Column is not a string type, skip
          end
        end
      end
      ...
      def self.check_duplicate_column_values(df, col, errors)
        ...
        # Get value counts, excluding nulls (they're reported separately as blank values)
