  ...
  module CLI
    class << self
      ...
      def read_compressed_full(file, options:)
        ...
        IO.popen(cmd) do |io|
          df = Formats.read_from_io(io,
            format: Formats.detect_format(file),
            delimiter: options[:delimiter],
            has_header: !options[:no_header],
          )
      ...
      def format_lint_issue(symbol, issue, df)
        ...
        # Add row data for issues with line numbers (limit to first 50 to avoid overwhelming output)
        if lines.any?
          display_lines = lines.take(50)
          message += "\n"
