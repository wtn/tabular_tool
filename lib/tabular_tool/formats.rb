  ...
  module Formats
    ...
    class << self
      ...
      def read_csv(path, delimiter:, has_header: true, streaming: false, **options)
        # Work around Polars bug: leading empty lines cause the header to be
        # treated as data. Only for uncompressed files (compressed handled separately).
        if !compressed_file?(path) && has_leading_empty_lines?(path)
          # Read file, skip leading empty lines, then pass to Polars
          File.open(path, 'r') do |file|
            read_csv_from_io(file, delimiter: delimiter, has_header: has_header, **options)
          end
        else
          if streaming
      ...
      def read_csv_from_io(io, delimiter:, has_header: true, **options)
        io.set_encoding("UTF-8:UTF-8") if io.respond_to?(:set_encoding)

        # Skip leading blank lines, then read the rest into a single buffer
        past_blanks = false
        ...
        require "stringio"
        Polars.read_csv(StringIO.new(buf), separator: delimiter, has_header: has_header, **options)
      end
