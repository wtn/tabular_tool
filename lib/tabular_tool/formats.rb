  ...
  module Formats
    ...
    class << self
      ...
      def write_to_stdout(df, format:, delimiter: nil, **options)
        case format
        when :csv
          df.write_csv(nil, sep: delimiter || ",")
        when :tsv
          df.write_csv(nil, sep: delimiter || "\t")
        when :json
          df.write_json
      ...
      def read_csv(path, delimiter:, has_header: true, streaming: false, **options)
        ...
        if !compressed_file?(path) && has_leading_empty_lines?(path)
          ...
          # Normal path - no preprocessing needed
          if streaming
            Polars.scan_csv(path, sep: delimiter, has_header: has_header, **options)
          else
            Polars.read_csv(path, sep: delimiter, has_header: has_header, **options)
          end
        end
      end

      def write_csv(df, path, delimiter:, **options)
        df.write_csv(path, sep: delimiter, **options)
      end

      def read_csv_from_io(io, delimiter:, has_header: true, streaming: false, **options)
        # Skip leading empty lines by reading from IO
        # Note: streaming mode not supported when preprocessing IO
        lines = []
        ...
        # Create a StringIO with the cleaned content
        require 'stringio'
        cleaned_io = StringIO.new(lines.join)

        # Polars can read from Ruby IO objects
        # Note: streaming not supported with IO input
        Polars.read_csv(cleaned_io, sep: delimiter, has_header: has_header, **options)
      end

      def has_leading_empty_lines?(path)
        # Check if file has leading empty lines
        File.open(path, 'r') do |file|
